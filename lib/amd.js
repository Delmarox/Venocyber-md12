...
Database:  ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "venocyber Default")}
\t_VARS_
...
<div class="bounce-text">Venocyber md running</div>
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^venocyber_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/venocyber;;;/gi, "").replace(/Venoctrl;;;/gi, "").replace(/Venoctrl;;;/gi, "").trim();
